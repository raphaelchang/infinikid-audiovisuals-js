...
vec4 gaussianBlur( sampler2D t, vec2 texUV, vec2 stepSize ){
	...
	for( int i = 0; i < stepCount; i++ ){
	    ...
	    vec4 col = texture2D( t, texUV + texCoordOffset ) + texture2D( t, texUV - texCoordOffset );
...
void main() {
  ...
  vec4 blur = gaussianBlur(tex0, uv, texelSize * direction);

  gl_FragColor = vec4(blur.rgb * colorScale, blur.a);
}
